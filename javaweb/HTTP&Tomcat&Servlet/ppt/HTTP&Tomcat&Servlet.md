...
* 响应头：第二行开始，格式为key：value形式
  ...
  ```properties
  Content-Type:表示该响应内容的类型，例如text/html，image/jpeg；浏览器根据响应内容的类型选择不同的解析引擎解析。
...
* 因为Tomcat支持Servlet/JSP规范，所以Tomcat也被称为Web容器、Servlet容器。Servlet需要依赖Tomcat才能运行。
  Servlet是一种动态的web资源，就是一个java的类，但是没有主方法(main方法)，不能独立运行，他需要依赖于Tomcat这种Web服务器才能运行起来。所以tomcat被称为servlet容器，它里面放了一堆servlet。 
...
* 打开`apache-tomcat-8.5.68`目录就能看到如下目录结构，每个目录中包含的内容需要认识下,
  ...
  webapps:就是以后项目部署的目录，webapps下的每个目录都是一个项目。
...
* Tomcat启动的时候，启动窗口一闪而过: 需要检查JAVA_HOME环境变量是否正确配置。tomcat是由java编写的。会用到本地的java环境。
...
* 开发完成部署的Web项目
  ...
  ![image-20230412190956579](C:\Users\wangyan\Desktop\2023cz\MyNote\javaweb\day08-HTTP&Tomcat&Servlet\ppt\assets\image-20230412190956579.png)
  ...
  * webapp下的资源文件会直接放到项目下。
  * 也就是说webapp这个目录就是完成部署后的项目目录，并且将不在webapp目录下的东西都放到webapp下，比如java、resource目录下的东西，并且java下的java文件要编译成class文件放到classes下，还有需要的jar包也要放到webapp下。然后将webapp改名(改成虚拟路径的名称)。
...
4. 将开发项目部署项目到Tomcat中
   ...
   * war exploded模式是将WEB工程以==当前文件夹的位置关系==(可以当成是地址引用)发布到Tomcat服务器上
...
* Servlet是JavaWeb最为核心的内容，它是Java提供的一门==动态==web资源(通过执行servlet的方法后再返回资源)开发技术。
...
* ==urlPattern配置规则==
  ...
  * 任意匹配
    ...
    1. 当我们的项目中的Servlet配置了 "/",会覆盖掉tomcat中的DefaultServlet(也是个servlet),当其他的url-pattern都匹配不上时都会走这个Servlet
    
    2. 当我们的项目中配置了"/*",意味着匹配任意访问路径
    
    3. DefaultServlet是用来处理静态资源的servlet，意思就是当你访问静态资源时会走这个servler。如果配置了"/"会把默认的覆盖掉，就会引发请求静态资源的时候没有走默认的而是走了自定义的Servlet类，最终导致静态资源不能被访问
