...
* 分析出现中文乱码的原因：
  ...
  * Tomcat在==获取流的==时候采用的编码是ISO-8859-1
...
```java
//设置响应的数据格式  及数据的编码
//设置响应的数据格式时顺带设置输出流的编码方式  如果不设置默认的输出流编码方式是ISO-8859-1
response.setContentType("text/html;charset=utf-8");
writer.write("你好");
//同时html的meta编码也要设置为utf-8
<meta charset="UTF-8"> //表示浏览器用什么解码方式
```
