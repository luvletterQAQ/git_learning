...
>【黑马程序员JavaWeb基础教程，Java web从入门到企业实战完整版】https://www.bilibili.com/video/BV1Qf4y1T7Hx?p=38&vd_source=37d44c19b90722aba78120f5ce91449b
...
* 各个数据库厂商去实现这套接口，提供数据库驱动jar包，驱动就是jdbc的实现类jar包。
...
>就是个工具类，里面的方法都是静态方法，是java提供的类。里面有两个重要方法：registerDriver和getConnection。
>
>注册驱动和获取connection。
...
* 开启事务
  ...
  > 参与autoCommit 表示是否自动提交事务，true表示自动提交事务，false表示手动提交事务。而开启事务需要将该参数设为为false。conn.setAutoCommit(false);这句代码就是开启事务。
...
> 重要的就三个方法：execute、executeQuery、executeUpdate

* 执行DDL、DML语句

  ![image-20210725175151272](assets/image-20210725175151272.png)

  >返回值是执行完DML语句后受影响的行数。执行完DDL语句，可能是0。
...
>   执行时就不用再进行这些步骤了，速度更快如果sql模板一样(select * from tb_user where username = ?)，则只需要进行一次检查、编译。
>
> * 防止SQL注入：==将敏感字符进行转义==
...
* 标准接口：==DataSource==
  ...
  ```java
  Connection getConnection() //这个DataSource接口就只有这一个方法   java提供的  各大厂商提供实现
